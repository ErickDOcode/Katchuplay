...
def criar():
    ...
    if imagem and imagem.filename:
        # imagem.filename can be Optional[str]; cast to str to satisfy type checkers
        filename = secure_filename(str(imagem.filename))
    ...
    if request.method == "POST":
        ...
        if 'imagem' in request.files:
            ...
            if imagem and imagem.filename != '':
                ...
                filename = f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}_{secure_filename(str(imagem.filename))}"
